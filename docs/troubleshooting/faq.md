...
### How do I update DClaw Calendar?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp calendar --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Calendar without Kubernetes?

Yes, for local development:

```bash
cd dclaw-calendar/frontend && npm run dev
cd dclaw-calendar/backend && uvicorn dclaw_calendar.main:app --reload
```
